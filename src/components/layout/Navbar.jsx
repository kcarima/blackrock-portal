...
export const Navbar = ({ company }) => {
  ...
  return (
    <nav className="fixed w-full z-50 bg-black border-b border-yellow-500/20 shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-24 items-center">
          <div className="flex items-center cursor-pointer" onClick={() => handleNav('/')}>
            <img src={company.logo} alt="BlackRock Logo" className="h-20 w-auto object-contain transition-transform hover:scale-105" />
          </div>

          <div className="hidden lg:flex space-x-6 items-center">
            {navItems.map(item => (
              <div key={item.id} className="relative group">
                <button onClick={() => !item.subItems && handleNav(item.id)} className={`flex items-center gap-1 text-sm font-bold tracking-wide transition-colors py-8 ${location.pathname === item.id ? 'text-yellow-500' : 'text-gray-300 hover:text-yellow-400'}`}>
                  {item.label} {item.subItems && <ChevronDown className="w-4 h-4" />}
                </button>
                {item.subItems && (
                  <div className="absolute left-0 top-[85px] w-56 bg-gray-900 border border-gray-800 rounded-b-md shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                    <div className="py-2">
            ...
            <div className="flex items-center gap-4 pl-4 ml-2 border-l border-gray-700">
